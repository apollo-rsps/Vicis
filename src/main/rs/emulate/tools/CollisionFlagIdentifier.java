...
/**
 * A simple command line utility to print the flags contained in a single value in a CollisionMatrix.
 *
 * @author Major
 */
public final class CollisionFlagIdentifier {
	...
	private enum CollisionFlag {
		 ...
		 */
		private CollisionFlag(int value) {
			this.value = value;
		}
	...
	private Stream<CollisionFlag> getFlags(int value) {
		return CollisionFlag.FLAGS.stream().filter(flag -> flag.set(value));
	}
	...
	private void print() {
		values.forEach(value -> {
			StringBuilder builder = new StringBuilder(Integer.toString(value)).append(" contains: ");
			builder.append(getFlags(value).map(CollisionFlag::name).collect(Collectors.joining(", ")));
